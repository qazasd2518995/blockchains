export const zhHans = {
  common: {
    loading: '载入中',
    error: '错误',
    save: '保存',
    cancel: '取消',
    confirm: '确认',
    close: '关闭',
    closeDialog: '关闭对话框',
    create: '创建',
    delete: '删除',
    edit: '编辑',
    freeze: '冻结',
    unfreeze: '解冻',
    reset: '重设',
    exit: '退出登录',
    search: '搜索',
    active: '启用中',
    frozen: '已冻结',
    disabled: '已停用',
    back: '返回',
    next: '下一页',
    prev: '上一页',
    actions: '操作',
    copy: '复制',
    refresh: '刷新',
    all: '全部',
    allStatus: '全部状态',
    status: '状态',
    more: '更多',
    empty: '暂无数据',
    logoutBtn: '登出',
    logoutAndSwitch: '登出／切换账号',
    language: '语言',
  },
  auth: {
    login: '登录',
    username: '代理账号',
    password: '密码',
    currentPassword: '目前密码',
    newPassword: '新密码',
    confirmNewPassword: '确认新密码',
    newPasswordHint: '新密码（至少 8 字，须含英文与数字）',
    accountSecurity: '账户安全',
    updatePassword: '更新密码',
    passwordUpdated: '密码已更新',
    currentPasswordRequired: '请输入目前密码',
    newPasswordMin: '新密码至少 8 字',
    newPasswordTooLong: '新密码过长',
    newPasswordLetter: '新密码须包含英文字母',
    newPasswordDigit: '新密码须包含数字',
    confirmPasswordRequired: '请再次输入新密码',
    newPasswordMismatch: '两次输入的新密码不一致',
    newPasswordSame: '新密码不可与目前密码相同',
    authenticate: '验证身份',
    authenticating: '验证中',
    invalidCreds: '账号或密码错误',
    secureChannel: '加密通道 · TLS 1.3',
    title: '代理后台',
    subtitle: '代理运营管理系统',
    protocol: '系统协议',
    requiresAuth: '仅限授权代理 · 公开注册已关闭',
    authorizedAgent: '授权代理',
    heroEyebrow: '代理后台',
    heroTitle: '代理管理平台，协助你掌握团队与会员账务。',
    heroDescription:
      '登录后可查看代理层级、会员资料、余额异动与报表记录，快速处理日常运营与账务确认。',
    heroChipHierarchy: '代理层级',
    heroChipMembers: '会员账务',
    heroChipReports: '报表记录',
    usernamePlaceholder: '请输入代理账号',
    captcha: '验证码',
    captchaPlaceholder: '4位数字',
    captchaReload: '重新产生验证码',
    captchaLoadFailed: '验证码载入失败，请重新整理页面',
    captchaRequired: '请先取得验证码',
    captchaDigits: '请输入4位数字',
    required: '必填',
  },
  shell: {
    linkLive: '连线 · 在线',
    node: '节点 03 / 俄勒冈-美国',
    session: '会话',
    super: '超级管理',
    terminal: '管理后台',
    operator: '操作员',
    balance: '余额',
    level: '层级',
    rebate: '退水',
    navigation: '导航',
    brand: '代理后台',
    brandShort: '管理后台',
    profile: '个人资料',
    changePassword: '更改密码',
  },
  nav: {
    dashboard: '仪表板',
    accounts: '账号管理',
    transfers: '点数转账',
    logs: '日志',
    reports: '报表统计',
    controls: '控制设定',
    audit: '审计日志',
    subAccounts: '子帐号',
    announcements: '公告管理',
  },
  dashboard: {
    title: '运营总览',
    subtitle: '代理运营仪表板',
    totalAgents: '下级代理',
    totalMembers: '下级会员',
    balance: '账户余额',
    commission: '佣金余额',
    rebate: '退水比例',
    level: '层级',
    status: '状态',
    recentActivity: '近期活动',
  },
  agents: {
    title: '账号管理',
    subtitle: '代理层级',
    mixedHierarchyBreadcrumb: '账号管理 · 混合层级',
    mixedHierarchySuffix: '混合层级',
    description: '点击代理账号进入下级；点击余额进行出入金。',
    createSub: '创建子代理',
    createMember: '创建会员',
    username: '账号',
    displayName: '显示名称',
    parentAgent: '上级代理',
    level: '层级',
    rebate: '退水 %',
    rebateMode: '退水模式',
    commission: '佣金率',
    bettingLimit: '下注额度',
    balance: '余额',
    memberCount: '会员数',
    subAgentCount: '子代理数',
    createdAt: '创建时间',
    lastLogin: '最后登录',
    notes: '备注',
    noChildren: '暂无子代理',
    emptyLevel: '— 此层级为空 —',
    confirmFreeze: '确定冻结此代理？所有下级会员也将受影响。',
    searchPlaceholder: '搜索账号或昵称',
    current: '当前代理',
    bal: '余额',
    rebatePct: '退水',
    directAgents: '直属代理',
    directMembers: '直属会员',
    type: '类型',
    account: '账号',
    typeAgent: '代理',
    typeMember: '会员',
    subAgents: '子代理',
    membersLabel: '会员',
    childAccounts: '下级账号',
    reports: '报表',
    logs: '日志',
    pointTransfer: '点数转移',
    enterDownline: '进入下级',
    balanceClickHint: '点击余额进行出入金',
    rebateSetup: '退水设定',
    resetPassword: '重设密码',
    betRecords: '下注记录',
    adjustBalance: '调整余额',
    notesBtn: '备注',
    statusMenu: '状态',
    enable: '启用',
    freezeAction: '冻结',
    disableAction: '停用',
    deleteAction: '删除',
    confirmFreezeMember: '冻结此会员？冻结后仍可登入，但不能下注或操作。',
    confirmFreezeMemberShort: '冻结此会员？冻结后仍可登入，但不能下注或操作。',
    confirmFreezeAgentTpl: '冻结代理 {name}？该账号仍可登入，但只能查看不能操作。',
    confirmDisableMemberShort: '停用此会员？停用后将无法登入。',
    confirmDisableAgentTpl: '停用代理 {name}？停用后将无法登入。',
    confirmDeleteAgent: '删除此代理？不可逆。',
    resetPasswordPromptTpl: '为 {name} 设置新密码（至少 8 字，须含英数）:',
    passwordReset: '密码已重设',
    editNotesPrompt: '编辑备注:',
    transferHint: '点数转移：请至「点数转账」页面。',
    rebateHint: '退水设定：请透过 API /agents/:id/rebate。',
    directSummaryTpl: '直属 {agents} 代理 + {members} 会员',
    sectionTag: '账号管理',
  },
  agent: {
    rebateModes: {
      PERCENTAGE: '按比例分配',
      ALL: '全拿退水',
      NONE: '全退下级',
    },
    status: {
      ACTIVE: '启用中',
      FROZEN: '已冻结',
      DISABLED: '已停用',
      DELETED: '已删除',
    },
  },
  tree: {
    children: '子代理',
    members: '会员',
    rebate: '退水',
  },
  members: {
    title: '会员管理',
    subtitle: '会员名册',
    create: '创建会员',
    email: '账号(邮箱)',
    username: '账号',
    displayName: '昵称',
    agent: '所属代理',
    balance: '余额',
    status: '状态',
    lastLogin: '最后登录',
    notes: '备注',
    bets: '下注记录',
    resetPassword: '重设密码',
    adjust: '调整点数',
    transfer: '转账',
    emptyList: '暂无会员',
    search: '搜索账号或昵称',
    password: '密码',
    initialBalance: '初始余额',
    confirmFreeze: '确定冻结此会员？冻结后无法下注登录。',
    confirmDelete: '确定删除此会员？不可逆。',
  },
  transfers: {
    title: '点数转账',
    subtitle: '点数转账记录',
    newTransfer: '新增转账',
    direction: '方向',
    deposit: '存入（代理→会员）',
    withdraw: '提领（会员→代理）',
    amount: '金额',
    description: '备注',
    from: '来源',
    to: '目标',
    before: '操作前',
    after: '操作后',
    timestamp: '时间',
    operator: '操作人',
    noTransfer: '暂无转账记录',
    member: '会员',
    currentBal: '当前余额',
    nextBal: '操作后余额',
  },
  logs: {
    title: '日志',
    subtitle: '点数操作记录',
    description: '记录当前登入代理执行的所有点数转移、入点与提取动作。',
    section: '§ 后台 05',
    breadcrumb: '日志 / 点数操作',
    refresh: '刷新',
    filter: '类型',
    allTypes: '全部行为',
    loaded: '目前载入',
    totalIn: '收回点数',
    totalOut: '转出点数',
    entries: '笔纪录',
    operator: '代理名',
    action: '行为',
    amount: '额度',
    counterparty: '对象',
    balance: '代理余额',
    descriptionLabel: '备注',
    noLogs: '暂无点数操作日志',
    loadMore: '加载更多',
    agentBalanceUnavailable: '不影响代理余额',
    actions: {
      AGENT_TRANSFER_OUT: '转给代理',
      AGENT_TRANSFER_IN: '代理提回',
      AGENT_TRANSFER: '代理间转账',
      MEMBER_DEPOSIT: '入点会员',
      MEMBER_WITHDRAW: '会员提取',
      CS_AGENT_ADJUST: '客服调整代理',
      CS_MEMBER_ADJUST: '客服调整会员',
      REBATE_PAYOUT: '退水派发',
      UNKNOWN: '点数操作',
    },
    typeFilters: {
      AGENT_TO_AGENT: '代理间转账',
      AGENT_TO_MEMBER: '入点会员',
      MEMBER_TO_AGENT: '会员提取',
      CS_AGENT_TRANSFER: '客服调整代理',
      CS_MEMBER_TRANSFER: '客服调整会员',
      REBATE_PAYOUT: '退水派发',
    },
  },
  adjust: {
    title: '调整余额',
    member: '会员',
    bal: '当前余额',
    delta: '调整金额（正数存入 / 负数扣除）',
    description: '备注说明',
  },
  bets: {
    game: '游戏',
    amount: '投注',
    multiplier: '倍率',
    payout: '派彩',
    profit: '盈亏',
    time: '时间',
    empty: '暂无下注记录',
  },
  controls: {
    win: '放水',
    loss: '杀分',
    active: '启用中',
    off: '已停用',
    capped: '已达上限',
    done: '已完成',
  },
  audit: {
    oldValues: '旧值',
    newValues: '新值',
  },
  reports: {
    from: '起始时间',
    to: '结束时间',
    rootAgentId: '根代理 ID',
    children: '下级统计',
  },
  router: {
    pathNotFound: '页面不存在',
  },
  // 后端错误码 → 中文讯息（依 code 对应，忽略英文 message）
  errors: {
    UNAUTHORIZED: '身份未授权,请重新登录',
    INVALID_CREDENTIALS: '账号或密码错误',
    EMAIL_TAKEN: '此邮箱已被使用',
    USER_NOT_FOUND: '找不到该用户',
    INSUFFICIENT_FUNDS: '余额不足',
    INVALID_BET: '下注设定不符合规则，请检查金额或选项',
    BET_OUT_OF_RANGE: '下注金额超出限红',
    GAME_DISABLED: '该游戏目前停用',
    ROUND_NOT_FOUND: '找不到本局数据',
    ROUND_NOT_ACTIVE: '本局已不接受操作',
    INVALID_ACTION: '此动作不允许',
    SEED_NOT_REVEALED: '会话尚未结束',
    RATE_LIMITED: '操作过于频繁,请稍候再试',
    INTERNAL: '系统内部错误,请联系管理员',
    USERNAME_TAKEN: '此账号已被使用',
    AGENT_NOT_FOUND: '找不到该代理',
    MEMBER_NOT_FOUND: '找不到该会员',
    AGENT_FROZEN: '代理已冻结,无法操作',
    MEMBER_FROZEN: '会员已冻结,无法操作',
    HIERARCHY_VIOLATION: '违反层级规则',
    REBATE_VIOLATION: '退水比例超过上级上限',
    INVALID_TRANSFER: '转账参数不合法',
    FORBIDDEN: '权限不足,无法执行此操作',
    // 服务端 message 关键字补充（backend 以英文文字回报时的 fallback）
    agentBalanceInsufficient: '代理余额不足',
    memberBalanceInsufficient: '会员余额不足',
    amountMustBePositive: '金额必须大于 0',
    amountCannotBeZero: '金额不能为 0',
    adjustmentGoNegative: '此调整会让余额变成负数',
    superAdminCannotFreeze: '超级管理员无法被冻结',
  },
};

export const dict = zhHans;

export type Dict = typeof zhHans;
