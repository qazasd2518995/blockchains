...
const GAME_POOL: Array<{
  ...
  { game: '爬階梯', gameId: 'tower', minMult: 2, maxMult: 12.5, minWin: 8000, maxWin: 120000 },
  {
    game: '小雞過馬路',
    gameId: 'chicken-road',
    minMult: 3,
  ...
  { game: '狂歡節', gameId: 'carnival', minMult: 2, maxMult: 12, minWin: 18000, maxWin: 380000 },
  {
    game: '水果拉霸',
    gameId: 'fruit-slot',
    minMult: 25,
...
export function reshuffleTop10(current: RankedWinRecord[]): RankedWinRecord[] {
  ...
  for (let index = 0; index < next.length; index += 1) {
    ...
    if (index > 2 && Math.random() < 0.45) {
      ...
      target.win = Math.max(
        replacement.win,
        roundWinAmount(replacement.win * randomBetween(1.05, 1.8)),
      );
    } else {
  ...
  if (Math.random() < 0.65) {
    ...
    challenger.win = Math.max(
      challenger.win,
      roundWinAmount(challenger.win * randomBetween(1.2, 2.4)),
    );
