...
┌──────────────────────────────────────────────────────────┐
│ TopBar — 黑底 #1A2530                                     │
│ [Logo] [優惠][充值][紀錄][驗證]...[a2518995][訊息][VIP1]    │
├──────────────────────────────────────────────────────────┤
│ AnnouncementTicker — 白底 / 紅字                          │
│ 📢 最新公告 ➤ ★儲值系統升級 ★官方公告 ★防詐騙 ...         │
├──────────────────────────────────────────────────────────┤
...
- 中：導覽連結 `優惠 / 充值 / 遊戲紀錄 / 驗證`
  - `/promos` — 優惠頁（展示用，可放活動卡片占位）
  - `/topup` — 充值頁（**不是真實金流**，僅讓 admin 測試用假幣加值；符合 CLAUDE.md「無購買點數、無提領」底線）
  - `/history` — 遊戲紀錄（已存在頁，保留）
  - `/verify` — Provably Fair 驗證頁（新增入口）
  - **下架**：「提款」「轉帳」連結（本專案無此功能；3A 原站有，但我們不抄）
- 右：
...
- 假公告可直接抄 3A 站（★儲值系統升級 / ★官方公告 / ★防詐騙 / ★USDT 入款通道 / ★金流部門通知 ...）
