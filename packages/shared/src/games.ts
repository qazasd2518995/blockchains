...
const H5_INDIVIDUAL_GAME_REGISTRY = Object.fromEntries(
  H5_GAMES.map((game) => [
    game.gameId,
    {
      ...
      description: `${game.title} original Cocos game connected to local platform settlement.`,
      descriptionZh: `${game.titleZh}原版 Cocos 遊戲，接入平台本地結算`,
      rtp: 0.95,
      icon: 'slots',
      enabled: true,
    },
...
export const GAMES_REGISTRY: Record<GameIdType, GameMetadata> = {
  ...
  [GameId.H5_SLOT_COLLECTION]: {
    ...
    category: 'single-step',
    description:
      'Twenty-one original Cocos slots and four fishing games connected to local platform settlement.',
    descriptionZh: '21 款原版 Cocos 拉霸與 4 款捕魚遊戲，接入平台本地結算',
    rtp: 0.95,
    icon: 'slots',
    enabled: false,
  },
